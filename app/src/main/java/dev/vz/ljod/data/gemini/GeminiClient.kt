        ...
        return withContext(Dispatchers.IO) {
            try {
                val body = buildJsonObject {
                    ...
                    put("messages", buildJsonArray {
                        add(buildJsonObject {
                            ...
                            put("content", JsonPrimitive(PromptTemplates.lyricsGeneration(title, artist, album)))
                        })
                    })
                ...
                val request = Request.Builder()
                    ...
                    .header("HTTP-Referer", "https://github.com/Veridian-Zenith/ljod")
                    .header("X-Title", "Ljod")
                    .build()

                client.newCall(request).execute().use { response ->
                    if (!response.isSuccessful) {
                        Timber.e("OpenRouter error: ${response.code}")
                        return@withContext null
                    }
                    val respStr = response.body?.string() ?: return@withContext null
                    val parsed = json.parseToJsonElement(respStr).jsonObject
                    val content = parsed["choices"]?.jsonArray
                        ?.firstOrNull()?.jsonObject?.get("message")?.jsonObject?.get("content")?.jsonPrimitive?.content
                        ?: return@withContext null
                    val cleaned = content.trim().removePrefix("```").removeSuffix("```").trim()
                    val lines = cleaned.lines()
                        .filter { it.isNotBlank() }
                        .map { LyricsLine(timeMs = 0, text = it.trim()) }
                    if (lines.isEmpty()) null else LyricsResult(lines = lines, source = "openrouter ($model)", isSynced = false)
                }
            } catch (e: Exception) {
                Timber.w(e, "OpenRouter lyrics generation failed")
                null
            }
        }
    }
            ...
            try {
                val request =
                    Request
                        ...
                        .header("User-Agent", "Ljod/0.2 (https://github.com/Veridian-Zenith/ljod)")
                        .get()
                        .build()
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    val parsed = json.decodeFromString(ListModelsResponse.serializer(), body)
                    parsed.models
                        .orEmpty()
                        .filter { model ->
                            val methods = model.supportedGenerationMethods.orEmpty()
                            ...
                            val isFree = outputLimit in 1..8192
                            AvailableModel(
                                id = rawId,
                                displayName =
                                    buildString {
                        ...
                        }.sortedWith(compareByDescending<AvailableModel> { it.isFree }.thenBy { it.id })
                }
            } catch (e: Exception) {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (lines.isEmpty()) {
                    ...
                    LyricsResult(lines = lines, source = "gemini ($model)", isSynced = false)
                }
            } catch (e: Exception) {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (lines.isEmpty()) {
                    ...
                    LyricsResult(lines = lines, source = "gemini_translation ($model)", isSynced = false)
                }
            } catch (e: Exception) {
            ...
            buildJsonObject {
                ...
                put(
                    ...
                    buildJsonArray {
                        add(
                            buildJsonObject {
                                put("category", JsonPrimitive("HARM_CATEGORY_HARASSMENT"))
                                put("threshold", JsonPrimitive("BLOCK_ONLY_HIGH"))
                            },
                        )
                        add(
                            buildJsonObject {
                                put("category", JsonPrimitive("HARM_CATEGORY_HATE_SPEECH"))
                                put("threshold", JsonPrimitive("BLOCK_ONLY_HIGH"))
                            },
                        )
                    },
