...
object Romanizer {
    ...
    private val scriptTransliterators = mapOf(
        "Han" to "Han-Latin",
    ...
    fun romanize(text: String): String {
        ...
        val ruleId = scriptTransliterators[script] ?: run {
    ...
    fun romanizeLyrics(lyrics: LyricsResult): LyricsResult {
        ...
        val anyChanged = romanizedLines.zip(lyrics.lines).any { (a, b) ->
    ...
    private val blockScriptMap = mapOf(
    ...
    private fun blockToScript(block: Character.UnicodeBlock?): String? {
        return blockScriptMap[block]
    }
}
